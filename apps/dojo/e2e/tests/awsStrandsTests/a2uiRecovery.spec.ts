...
test("[AWS Strands] A2UI recovery — exhaustion never paints a faulty surface, chat stays usable", async ({
  page,
}) => {
  await page.goto("/aws-strands/feature/a2ui_recovery");
...
test("[AWS Strands] A2UI recovery — exhaustion shows the hard-failure UI", async ({
  page,
}) => {
  await page.goto("/aws-strands/feature/a2ui_recovery");
